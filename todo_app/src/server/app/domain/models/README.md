...
type DiContainer* = tuple
  ...
  # userRdbRepository: UserJsonRepository
```

In this example, `Repository Interface` call `UserRdbRepository` by resolving as `userRdbRepository`.
...
proc find*(this:IUserRepository, email:Email):Option[User] =
  return DiContainer.userRdbRepository().find(email)

proc save*(this:IUserRepository, user:User):int =
  return DiContainer.userRdbRepository().save(user)
```
